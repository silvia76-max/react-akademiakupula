...
const getAuthToken = () => localStorage.getItem('akademia_auth_token');
...
export const getUserSessions = async (userId, page = 1, perPage = 50) => {
  try {
    const response = await axios.get(`${API_URL}/users/${userId}/sessions?page=${page}&per_page=${perPage}`);
    const data = response.data.data;
    return Array.isArray(data) ? data : [];
  } catch {
    return [];
